...
/* DotGridAvatar — canvas-based, hex-grid dot animation for the agent avatars.
 *
 * Three intensity fields:
 *   pulse    — radial rings expanding from the center, calm and organic.
 ...
 * anything in between) without prop changes. */
...
export type DotGridAvatarProps = {
  ...
  /* When true, the animation freezes at t=0 (used for idle agents so the
   * signature shape stays legible without competing motion). */
  paused?: boolean;
};
...
const summaryCurve: { samples: Float64Array | null; R: number } = {
  samples: null,
  R: -1,
};
...
const FIELDS: Record<DotGridPattern, Field> = {
  ...
  stars(dx, dy, t, R, T) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const sigBody = R * (0.05 + 0.04 * localEnv);
